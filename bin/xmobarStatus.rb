...
class CPU
  ...
  def sample
    now, current = Time.now, File.open("/proc/stat").each_line.reduce([]) do |idles, l|
    ...
    if @then
      ...
      per_core = idles[1..-1].map{|i| u=used(i); pcolored(u, "%d"%(u/10))}
      "C#{pcolored(total, "%2d"%total)}(#{per_core.join})"
    end
  ensure
    @then, @previous = now, current
  end
end
...
class Network
  ...
  def sample
    now, current = Time.now, File.open("/proc/net/dev").readlines.grep(/^ *[we]/).reduce([0,0]) do |rt, l|
      tokens = l.split
      [rt[0] + tokens[1].to_i, rt[1] + tokens[9].to_i]
    end
    if @then
      dt = now - @then
      r, t = current.zip(@previous).map{|c,p| b=(c-p)/dt; ncolored(b, ("%.3f"%(b/1e6))[0..4])}
      "#{r}RT#{t}"
    end
  ensure
    @then, @previous = now, current
...
class Disk
  ...
  def sample
    now, current = Time.now, File.open("/proc/diskstats").readlines.grep_v(/ 0 (nvm|mm|hd|sd)/).reduce([0,0]) do |rw, l|
      tokens = l.split
      [rw[0] + 512*tokens[5].to_i, rw[1] + 512*tokens[9].to_i]
    end
    if @then
      dt = now - @then
      r, t = current.zip(@previous).map{|c,p| b=(c-p)/dt; dcolored(b, ("%.1f"%(b/1e6))[0..2])}
      "#{r}RW#{t}"
    end
  ensure
    @then, @previous = now, current
