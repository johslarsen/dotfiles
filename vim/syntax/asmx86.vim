...
" syn region asmx86Comment	start="#\(\(\(include\)\|\(define\)\|\(if\)\|\(ifndef\)\|\(endif\)\|\(end\)\|\(ifdef\)\)\s\)\!" end="$"
 ...
 syn region asmx86Commenthash	start="#\(\(define\)\|\(if\(n\=\(def\)\)\=\)\|\(include\)\|\(error\)\|\(end\(if\)\=\)\)\!" end="$" contains=asmx86Todo,@Spell

syn region asmx86Commenthash	start="#" end="$" contains=asmx86Todo,@Spell
...
syn region asmx86Commentcpp	start="\s*#\s*if\s\+0\+\>" end="\s*#\s*endif\>" contains=asmx86Todo,@Spell
syn region asmx86Commentcxx	start="//" end="$" contains=asmx86Todo,@Spell
syn region asmx86Commentc	start="/\*" end="\*/" contains=asmx86Todo,@Spell
