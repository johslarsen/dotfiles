...
vim.keymap.set('n', '<Leader>g', function() fzf.grep_cword({ cmd = "git grep -n --column", filespec = vim.fn.expand("%:h") }) end)
vim.keymap.set('v', '<Leader>g', function() fzf.grep_visual({ cmd = "git grep -n --column", filespec = vim.fn.expand("%:h") }) end)
